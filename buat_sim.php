...
if(empty($_FILES['surat_pindah']['name'])){
    $surat_pindah="";
}else{
    $surat_pindah = "SPKA".rand(pow(10, $digits-1), pow(10, $digits)-1);
}

$source3 = $_FILES['surat_pindah']['tmp_name'];

if(empty($_FILES['surat_jaminan']['name'])){
    $surat_jaminan="";
}else{
    $surat_jaminan="SJBT".rand(pow(10, $digits-1), pow(10, $digits)-1);
}

$source4 = $_FILES['surat_jaminan']['tmp_name'];

$folder = './images/';
move_uploaded_file($source1,$folder.$fckk);
move_uploaded_file($source2,$folder.$fc_ktp);
move_uploaded_file($source3,$folder.$surat_pindah);
move_uploaded_file($source4,$folder.$surat_jaminan);


$sql = "INSERT INTO buat_sim (nama, nik, pekerjaan, alamat,fc_kk, fc_ktp, surat_pindah, surat_jaminan, input_by, id_data) values('$nama', '$nik','$pekerjaan','$alamat','$fckk','$fc_ktp','$surat_pindah','$surat_jaminan', '$nama_username', '$id_data')";
...
try {
    ...
    $mail->Subject = 'Record Formulir Pembuatan Surat Izin Menetap';
