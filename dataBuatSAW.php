...
if($_SESSION['akun_level'] == "admin"){
    $sql= "SELECT * FROM buat_saw";
    $result = $connection->query($sql);
    $result;
}else{
$sql= "SELECT * FROM buat_saw WHERE input_by='$nama_username'";
$result = $connection->query($sql);
$result;
}
        ...
        <div class="container-fluid" style="background-color: white; border-radius: 10px; width: 350%">
            ...
            <table class="table" id="myTable">
                <thead>
                    ...
                    <th scope="col">KTP Ahli Waris</th>
                    <th scope="col">KTP Saksi</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo $row['alamat_ahli_waris'];?></td>
                        <td><?php echo $row['nama_saksi'];?></td>
                        <td><?php echo $row['tanggal_input'];?></td>

                        <td><?php if($row['kk_ahli_waris'] !=""){?><a href="images/<?php echo $row['kk_ahli_waris'];?>">Lihat KK</a><?php }else{echo "Tidak ada KK";}?></td>
                        <td><?php if($row['ktp_ahli_waris'] !=""){?><a href="images/<?php echo $row['ktp_ahli_waris'];?>">Lihat KTP</a><?php }else{echo "Tidak ada KTP";}?></td>
                        <td><?php if($row['ktp_saksi'] !=""){?><a href="images/<?php echo $row['ktp_saksi'];?>">Lihat KTP</a><?php }else{echo "Tidak ada KTP";}?></td>
                        <td><?php if($row['surat_kematian'] !=""){?><a href="images/<?php echo $row['surat_kematian'];?>">Lihat Surat</a><?php }else{echo "Tidak ada";}?></td>
                        ...
                        <?php 
                            ...
                            "<td align='center'>";
                            if($_SESSION['akun_level'] != "admin"){
                                if($row['status']!="Diterima"){echo "<a id='edit_data' data-toggle='modal' data-target='#edit-data' data-id_data='".$row['id']."'"."data-status='".$row['status']."'"."data-id_data2='".$row['id_data']."'"."data-nama='".$row['nama']."'"."data-nik='".$row['nik']."'"."data-pekerjaan='".$row['pekerjaan']."'"."data-alamat='".$row['alamat_ahli_waris']."'"."data-nama_saksi='".$row['nama_saksi']."'".">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body" id="modal-edit">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama_saksi" class="form-control" id="nama_saksi" required>
            </div>
        </div>
        <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
$(document).on("click", "#edit_data", function() {
    ...
    var nama_saksi = $(this).data('nama_saksi');    
    ...
    $("#modal-edit #nama_saksi").val(nama_saksi);  
})
$(document).ready(function(e) {
    $("#form_edit_data").on("submit", (function(e) {
    e.preventDefault();
    $.ajax({
        url:'edit_data_saw.php',
        type: 'POST',
    ...
    window.location="?page=dataBuatSAW";
    }));
});
